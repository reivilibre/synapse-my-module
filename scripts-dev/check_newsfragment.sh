...
# If towncrier returns a non-zero exit code, exit
python -m towncrier.check --compare-with="origin/master" || exit 1
...
matched=0
for f in `git diff --name-only origin/master... -- changelog.d`; do
